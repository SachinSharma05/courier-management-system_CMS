...
export default function PricingPage() {
  ...
  // 2. Data Fetching Hook
  const { data, isLoading, refetch } = useRateCard(provider, service, clientId) as any;
...
function RateRow({ label, slabType, rates, rateCardId, isHighlight, gst }: any) {
  return (
    <tr className={clsx("transition-colors group", isHighlight ? "bg-indigo-50/20" : "hover:bg-slate-50/50")}>
      <Td className="font-bold text-slate-700 pl-6 border-r border-slate-100 text-sm">{label}</Td>
      {ZONES.map((zone) => (
        <Td key={zone} className="text-center border-l border-slate-100 p-0">
          <EditableRateCell
            initialValue={rates[zone] ?? 0}
...
function EditableRateCell({ initialValue, rateCardId, zoneCode, slabType, gstEnabled }: any) {
  const [val, setVal] = useState(initialValue);
  const [isEditing, setIsEditing] = useState(false);
  const [status, setStatus] = useState<'idle' | 'saving' | 'success'>('idle');
...
function HeaderButton({ icon, label, variant }: any) {
  return (
    <button className={clsx(
...
function FooterNote({ icon, text, color }: any) {
  const styles = color === 'indigo' ? "bg-indigo-50 text-indigo-700 border-indigo-100" : "bg-emerald-50 text-emerald-700 border-emerald-100";
  return (
