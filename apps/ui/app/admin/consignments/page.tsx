...
export default function ConsignmentsPage() {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#F8FAFC] min-h-screen text-slate-900">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between bg-white p-5 border border-slate-200 rounded-lg">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-600 rounded-lg">
            <Box size={24} />
          </div>
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900">Shipment Ledger</h1>
            <p className="text-xs text-slate-500 font-medium mt-1">Operational control and logistics tracking</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden relative">
        ...
        <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <PaginationBtn onClick={() => setPage(1)} disabled={page === 1} icon={ChevronsLeft} />
            <PaginationBtn onClick={() => setPage(p => p - 1)} disabled={page === 1} icon={ChevronLeft} />
            <span className="text-xs font-bold px-3 py-1 bg-white border border-slate-200 rounded-md text-slate-900">
              {page} / {totalPages}
            </span>
            <PaginationBtn onClick={() => setPage(p => p + 1)} disabled={page >= totalPages} icon={ChevronRight} />
            <PaginationBtn onClick={() => setPage(totalPages)} disabled={page >= totalPages} icon={ChevronsRight} />
          </div>
        </div>
      </div>
...
function PaginationBtn({ icon: Icon, onClick, disabled }: any) {
  return (
    <button 
      onClick={onClick} 
      disabled={disabled} 
