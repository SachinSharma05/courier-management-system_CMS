    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Initialize Shipment Node</h1>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.15em] flex items-center gap-2 mt-1">
              <Terminal size={12} className="text-blue-500" /> Target_Client: #{clientId} <span className="text-slate-200">|</span> Provider: DTDC_EXPRESS
            </p>
          </div>
        </div>
        <div className="h-9 rounded-full border border-slate-200 font-bold text-[10px] px-4 bg-white text-slate-500 flex items-center gap-2 shadow-sm">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
             <div className="px-6 py-4 border-b border-slate-50 bg-slate-50/30 flex items-center gap-3">
                ...
                <h2 className="text-[11px] font-black uppercase tracking-widest text-slate-800">Validation Protocols</h2>
             </div>
             
             <div className="p-6 space-y-3">
                <ProtocolRow icon={<Hash size={14}/>} label="Pincode Check" status="6-Digit Numeric" />
                <ProtocolRow icon={<Package size={14}/>} label="Weight Metric" status="Unit: Kilograms (KG)" />
                <ProtocolRow icon={<Database size={14}/>} label="API Endpoint" status="Live Production" />
                
                <div className="mt-4 p-5 bg-blue-50/50 border border-blue-100 rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    <Info size={14} className="text-blue-600" />
                    <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Operator Note</span>
                  </div>
                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed uppercase tracking-tight">
                    Detailed address strings prevent NDR flags. Ensure house/office numbers are present.
                  </p>
                </div>
             </div>
          </div>

          <div className="p-8 bg-[#0F172A] rounded-2xl shadow-2xl shadow-blue-900/10 text-white space-y-6 relative overflow-hidden">
             <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
                <Activity size={16} className="text-blue-400" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">Real-Time Manifest</span>
             </div>
             ...
             <Hash size={80} className="absolute -bottom-4 -right-4 text-white/5 rotate-12" strokeWidth={3} />
          </div>
        </div>

        {/* ───────────────── RIGHT: WORKSPACE FORM ───────────────── */}
        <div className="lg:col-span-8 bg-white border border-slate-100 rounded-2xl shadow-sm">
          <form 
            onSubmit={(e) => { e.preventDefault(); submitBooking(); }} 
          ...
          >
            {/* Consignee Identity */}
            <div className="space-y-6">
              <SectionHeader icon={<User size={16}/>} title="Consignee Identity Profile" />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormGroup 
            ...
            {/* Logistics Metrics */}
            <div className="space-y-6">
              <SectionHeader icon={<MapPin size={16}/>} title="Logistics Destination Metrics" />
              <div className="space-y-6">
                <FormGroup 
            ...
            {/* Metadata */}
            <div className="space-y-6">
               <SectionHeader icon={<Layers size={16}/>} title="Reference Metadata" />
               <FormGroup 
                  label="Internal Order ID (Optional)" 
                  value={form.reference} 
                  onChange={(val: string) => setForm({ ...form, reference: val })}
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full flex items-center justify-center gap-4 rounded-2xl bg-blue-600 h-16 text-xs font-black text-white hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 active:scale-[0.99] disabled:opacity-50 uppercase tracking-[0.2em]"
            >
              {isPending ? (
                <>
                  <Loader2 className="animate-spin" size={20} />
                  Broadcasting Booking Packet...
                </>
              ) : (
                <>
                  Commit Booking Sequence
                  <Send size={18} />
                </>
              )}
            </button>
...
function SectionHeader({ icon, title }: { icon: any, title: string }) {
  return (
    <div className="flex items-center gap-3 border-b border-slate-50 pb-4">
      <div className="text-blue-600">{icon}</div>
      <h2 className="text-xs font-black uppercase tracking-[0.15em] text-slate-800">{title}</h2>
    </div>
  );
}
      ...
      <div className="relative group">
        ...
        {isTextArea ? (
          <textarea
            ...
            onChange={(e) => onChange(e.target.value)}
            placeholder={placeholder}
            className="w-full rounded-xl border border-slate-100 bg-slate-50/50 p-4 text-[13px] font-bold outline-none focus:bg-white focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all resize-none pl-12"
          />
        ) : (
          <Input
            required
            ...
            placeholder={placeholder}
            className={clsx(
              "h-14 rounded-xl border-slate-100 bg-slate-50/50 text-[13px] font-bold focus:bg-white focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all",
              icon && "pl-12"
            )}
          />
        )}
...
function ProtocolRow({ icon, label, status }: any) {
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-50/50 rounded-xl border border-slate-50">
...
function ManifestLine({ label, value }: any) {
  return (
    <div className="flex justify-between items-center text-xs">
      ...
      <span className="text-blue-400 font-bold uppercase truncate max-w-[180px] tracking-tight italic">{value}</span>
    </div>
  );
}