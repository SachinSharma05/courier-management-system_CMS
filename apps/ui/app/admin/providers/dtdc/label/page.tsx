...
import { 
  Printer, Download, Loader2, ExternalLink, ShieldCheck, 
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Print Queue Terminal</h1>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.15em] flex items-center gap-2 mt-1">
              <Terminal size={12} className="text-blue-500" /> SYSTEM: DTDC_LABEL_GEN_V2 <span className="text-slate-200">|</span> MODE: THERMAL_OPTIMIZED
            </p>
          </div>
        </div>
        ...
        {hasLabel && (
          <Button 
            onClick={download}
            className="rounded-2xl bg-blue-600 hover:bg-blue-700 text-white gap-3 h-14 px-8 shadow-xl shadow-blue-100 font-black text-[11px] uppercase tracking-[0.2em] transition-all active:scale-95"
          >
            <Download size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-sm space-y-8">
            <div className="flex items-center gap-3 border-b border-slate-50 pb-4">
              ...
              <h2 className="text-xs font-black uppercase tracking-[0.15em] text-slate-800">Input Parameters</h2>
            </div>

            <div className="space-y-2 group">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 group-focus-within:text-blue-500 transition-colors">
                WAYBILL_ID / AWB_NODE
              </Label>
              <div className="relative">
                ...
                <Input 
                  ...
                  onChange={(e) => setAwb(e.target.value)}
                  placeholder="EX: D12345678"
                  className="pl-12 h-14 rounded-xl border-slate-100 bg-slate-50/50 focus:bg-white font-mono font-bold text-[13px] transition-all uppercase tracking-widest"
                />
              </div>
            </div>
            ...
            <Button 
              onClick={generate} 
              disabled={loading || !awb}
              className="w-full h-16 rounded-2xl bg-[#0F172A] hover:bg-black text-white font-black text-[11px] uppercase tracking-[0.3em] shadow-xl shadow-slate-200 transition-all active:scale-[0.98]"
            >
              {loading ? <Loader2 className="animate-spin mr-3" size={18} /> : <Database className="mr-3" size={18} />}
              {loading ? "Establishing Sync..." : "Fetch Node Label"}
            </Button>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-[2rem] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-50 bg-slate-50/50 flex items-center gap-3">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-800">Printing Protocols</h3>
            </div>
          ...
          <div className="bg-amber-50/50 border border-amber-100 p-6 rounded-[1.5rem] flex gap-4">
             <AlertCircle className="text-amber-600 shrink-0" size={20} />
             <p className="text-[10px] font-bold text-amber-800 leading-relaxed uppercase tracking-widest italic opacity-80">
               Warning: Ensure thermal head is clean. Illegible waybills may cause terminal routing delays.
             </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          {hasLabel ? (
            <div className="bg-white border border-slate-100 rounded-[2.5rem] shadow-sm overflow-hidden animate-in slide-in-from-right-4 duration-700">
              <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                <div className="flex items-center gap-5">
                  <div className="flex items-center gap-2">
                    <Monitor size={16} className="text-blue-600" />
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Preview Stream</span>
                  </div>
                  <div className="h-4 w-[1px] bg-slate-200" />
                  <span className="font-mono text-sm font-black text-slate-900 tracking-tighter italic bg-slate-100 px-3 py-1 rounded-lg">
                    NODE_ID: {awb}
                  </span>
                </div>
                <Button 
                    ...
                    size="sm" 
                    onClick={() => window.open(pdfUrl || `data:application/pdf;base64,${pdfBase64}`)} 
                    className="h-11 rounded-xl border-slate-200 text-blue-600 font-black text-[9px] uppercase tracking-[0.15em] gap-2 hover:bg-blue-50 transition-all"
                >
                  <ExternalLink size={14} /> View Original Buffer
                </Button>
              </div>
              
              <div className="p-10 bg-slate-50/30 relative">
                <div className="border border-slate-200 rounded-[2rem] overflow-hidden shadow-2xl bg-white min-h-[600px] flex items-center justify-center relative z-10 ring-8 ring-white">
                  <iframe
                    className="w-full h-[700px]"
                    src={pdfUrl ? pdfUrl : `data:application/pdf;base64,${pdfBase64}`}
                    title="DTDC Label Preview"
            ...
            <div className="h-full min-h-[600px] border-2 border-dashed border-slate-100 rounded-[3rem] flex flex-col items-center justify-center text-center p-12 bg-slate-50/30 relative overflow-hidden group">
               <Monitor size={300} className="absolute opacity-[0.02] text-slate-900 group-hover:scale-110 transition-transform duration-1000" />
              ...
              <div className="relative z-10">
                <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Awaiting_Buffer_Data</h3>
                <p className="text-slate-400 max-w-xs mt-4 text-[11px] font-bold uppercase tracking-widest leading-relaxed opacity-60">
                  Initialize the thermal label rendering sequence by entering a Waybill ID in the control panel.
                </p>
              </div>
            </div>
          )}
...
function GuidelineItem({ text }: { text: string }) {
  return (
    <div className="flex gap-4 group">
      <div className="mt-1 w-2.5 h-2.5 bg-blue-500 shrink-0 rounded-full shadow-[0_0_12px_rgba(59,130,246,0.6)] group-hover:scale-125 transition-transform" />
      <p className="text-[10px] font-black text-slate-500 uppercase tracking-tight leading-relaxed group-hover:text-slate-900 transition-colors">{text}</p>
    </div>
  );
}