...
export default function EmployeesPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-hidden">
      ...
      {/* ───────────────── HEADER ───────────────── */}
      <header className="border-b border-slate-200 bg-white px-6 py-4 flex items-center justify-between shadow-sm z-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black tracking-tight uppercase">Staff Command Center</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Operations & Human Resources</p>
          </div>
        </div>
...
function EmployeeDrawer({ type, employee, onClose, onSaved }: { type: DrawerType, employee: Employee | null, onClose: () => void, onSaved: () => Promise<void> }) {
  ...
  const [formData, setFormData] = useState({ 
    employee_code: '', name: '', email: '', designation: 'Staff', 
    phone: '', department: 'Operations', base_salary: 0, 
    joining_date: new Date().toISOString().slice(0, 10), 
    salary_type: 'Monthly', is_active: true 
  });
  ...
  const handleAction = async () => {
    setLoadingAction(true);
    try {
      if (type === 'add') await createEmployee(formData as any);
      if (type === 'edit' && employee) await updateEmployee(employee.id, formData as any);
      if (type === 'holidays') await createHoliday(holidayData);
      if (type === 'attendance' && employee) await markAttendance({ employee_id: employee.id, date: selectedDate, status: selectedStatus });
      if (type === 'advance' && employee) await createAdvance({ employee_id: employee.id, amount, remarks });
...
function InputField({ label, value, onChange, type = "text", placeholder = "" }: { label: string, value: any, onChange: (v: string) => void, type?: string, placeholder?: string }) {
  return (
